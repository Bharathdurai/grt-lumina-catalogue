...
const Index = () => {
  ...
  const { data: allProducts = [] } = useCatalogueProducts({ includeOutOfStock: false });
  ...
  return (
    <div>
      ...
      <section className="relative h-[70vh] md:h-[80vh] overflow-hidden">
        <img src={heroBanner} alt="GRT Jewellery Collection" className="absolute inset-0 w-full h-full object-cover" />
        ...
        <div className="relative container mx-auto px-4 h-full flex items-center">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="max-w-lg">
            ...
            <div className="flex gap-4 flex-wrap">
              <Link to="/shop" className="inline-flex items-center gap-2 gradient-gold text-primary-foreground px-6 py-3 rounded-lg font-body font-semibold hover:opacity-90 transition-opacity">
                Explore Collection <ArrowRight size={18} />
              </Link>
              <Link to="/about" className="inline-flex items-center gap-2 border border-primary-foreground/40 text-primary-foreground px-6 py-3 rounded-lg font-body hover:bg-primary-foreground/10 transition-colors">
                Our Story
              </Link>
            </div>
          </motion.div>
      ...
      <section className="gradient-gold py-10">
        <div className="container mx-auto px-4 flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <Link to="/shop" className="bg-primary-foreground text-primary px-6 py-3 rounded-lg font-body font-semibold hover:opacity-90 transition-opacity">
            Shop Gold →
          </Link>
        </div>
      </section>
      ...
      {featured.length > 0 && (
        <section className="py-16 md:py-24 container mx-auto px-4">
          ...
          <div className="text-center mt-10">
            <Link to="/shop" className="inline-flex items-center gap-2 font-body font-semibold text-primary hover:underline">
